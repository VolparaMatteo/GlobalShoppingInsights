...
export async function updateWPConfig(
  payload: WPConfigUpdate,
): Promise<WPConfig> {
  const { data } = await client.patch<WPConfig>(
    "/settings/wordpress",
    payload,
  );
  return data;
...
export async function addBlacklist(
  ...
  const { data } = await client.post<BlacklistEntry>(
    "/settings/blacklist",
    payload,
  );
  return data;
...
export async function updateScrapingSettings(
  ...
  const { data } = await client.patch<ScrapingSettings>(
    "/settings/scraping",
    payload,
  );
  return data;
...
export async function updateDedupSettings(
  payload: Partial<DedupSettings>,
): Promise<DedupSettings> {
  const { data } = await client.patch<DedupSettings>(
    "/settings/dedup",
    payload,
  );
  return data;
