...
export async function addComment(articleId: number, payload: CommentCreate): Promise<Comment> {
  const { data } = await client.post<Comment>(`/articles/${articleId}/comments`, payload);
  return data;
}
